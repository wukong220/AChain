   ...
   do
      ...
      do   
         ...
         then
            ...
            echo "... mpirun -np 1 lmp_wk -l $t.test.log -i $t.act.testin ..."
         else
            sed "15s/2.0/${S}/;16,23s/0.5/${R}/;36s/1.0/${Fa}/;34s/1.0/${Kb}/;67s/2.0/$a/;\
            ...
            #rename
            mv init.100000.restart ${t}.init.restart
            bsub -J ${Phi}${D}$t -n 1 mpirun lmp_wk -l $t.log -i $t.act.in > $t.data                                      #NJU
            ...
            echo " --- bsub -J ${Phi}${D}$t -n 1 mpirun lmp_wk -l $t.log -i $t.act.in > $t.data --- "
            cd ../../
         fi
       done  
     done
