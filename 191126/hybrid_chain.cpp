...
const int N_atomtypes = 2 + 1;				//Number of atom types
...
int main()
{
	...
	string filename;
	stringstream ss;
	ss << N_chain << "N_" << Num_chains << "Chain.data";
	ss >> filename; 
	...
	for (int i = 0; i < Num_chains; i++)
	{
		...
		{
			chain[i][0] = l/2.0;	
			//chain[i][0] = L_box * Rand.doub() - 0.5 * L_box;
			chain[i][1] = l/2.0;	
			//chain[i][1] = L_box * Rand.doub() - 0.5 * L_box;
		}
		else
		{
