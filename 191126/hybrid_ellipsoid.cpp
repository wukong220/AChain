...
const double density = 1.0;			//Ellipsoidal density
const double D = 1.0;
const double S = 2.0;
const double ax = S * D;				//shapex
...
const int Num_ellipsoids = number(ax, shape[1], L_box, Phi_ellipsoids);		//Number of the ellipsoids
...
int main()
{
	...
	stringstream ss;
	string filename;
	ss << fixed << setprecision(2) << Phi_ellipsoids << setprecision(1) << "Phi_" << S << "S_" << D << "D.data";
	ss >> filename;
	//cout << filename;cin.get();
